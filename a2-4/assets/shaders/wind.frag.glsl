...
vec2 distort(vec2 uv, float t) {
	...
	border = smoothstep(0.0, 0.1, border); // Smooth falloff near edges
	
	float wave = sin(uv.y * 5.0 + t);
	uv.x += wave * 0.025 * border; // Reduce distortion near borders
...
vec3 colorShift(vec3 in_color) {
	...
	in_color.x *= 1;
	in_color.y *= 1;
	in_color.z *= 1,2; // We eincrement blue by 20%
